...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>  
	<div class="single-post">
		<h1><?php the_title();?></h1>
		<?php if( get_theme_mod( 'vw_restaurant_lite_single_postdate',true) == 1 || get_theme_mod( 'vw_restaurant_lite_single_author',true) == 1 || get_theme_mod( 'vw_restaurant_lite_single_comments',true) == 1 || get_theme_mod( 'vw_restaurant_lite_single_time',true) == 1) { ?>
			<div class="metabox">
				<?php if(get_theme_mod('vw_restaurant_lite_single_postdate',true)==1){ ?>
				...
				<?php if(get_theme_mod('vw_restaurant_lite_single_author',true)==1){ ?>
			      <span><?php echo esc_html(get_theme_mod('vw_restaurant_lite_meta_field_separator'));?></span> <i class="fas fa-user"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?></a></span>
			    <?php } ?>

			    <?php if(get_theme_mod('vw_restaurant_lite_single_comments',true)==1){ ?>
			      <span><?php echo esc_html(get_theme_mod('vw_restaurant_lite_meta_field_separator'));?></span> <i class="fas fa-comments"></i><span class="entry-comments"><?php comments_number( __('0 Comments','vw-restaurant-lite'), __('0 Comments','vw-restaurant-lite'), __('% Comments','vw-restaurant-lite')); ?></span>
			    <?php } ?>

			    <?php if(get_theme_mod('vw_restaurant_lite_single_time',true)==1){ ?>
		            <span><?php echo esc_html(get_theme_mod('vw_restaurant_lite_meta_field_separator'));?></span> <i class="fas fa-clock"></i><span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
	          	<?php } ?>
			</div>
		<?php } ?>
		...
		<?php if(has_post_thumbnail()) { ?>
	        ...
	        <hr>                    
	    <?php } ?> 
	     <?php if( get_theme_mod( 'vw_restaurant_lite_single_post_category',true) == 1) { ?>
	        <div class="single-post-category mt-3">
	            <span class="category"><?php esc_html_e('Categories:','vw-restaurant-lite'); ?></span>
	            <?php the_category(); ?>
	        </div> 
    	<?php }?>
